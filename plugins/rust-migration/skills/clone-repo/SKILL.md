...
Clone a git repository to a specified location with comprehensive validation, error handling, and
status reporting.

## Required Permissions

⚠️ **CRITICAL**: This skill requires network access and git operations.
...
echo "Initializing git operations with full permissions"
```
...
location, handling various scenarios and edge cases gracefully.

### Step 1: Gather Required Information

If not already provided by the user, ask for:

1. **Repository URL** ($ARGUMENTS[0]):
...
2. **Destination Directory** ($ARGUMENTS[1]):
...
Check if the repository already exists at `$ARGUMENTS[1]/<repo_name>`:

1. **If directory exists AND contains `.git` subdirectory**:
   - Navigate to the directory: `cd $ARGUMENTS[1]/<repo_name>`
...
2. **Build clone command** based on parameters:
   ...
   - Add source and destination: `$ARGUMENTS[0] $ARGUMENTS[1]/<repo_name>`

3. **Execute clone**:

   ```bash
   git clone [options] $ARGUMENTS[0] $ARGUMENTS[1]/<repo_name>
   ```

   Note: Permissions already established in Step 1a.
...
Location: $ARGUMENTS[1]/<repo_name>
Branch: <branch_name>
